...
export default function GenreGrid() {
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <h2 className="mb-8 font-display text-2xl font-bold text-ink-800 dark:text-parchment-100">
        Browse by Genre
      </h2>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {GENRES.map((genre) => (
          <Link
            key={genre.name}
            href={`/ebooks?genre=${encodeURIComponent(genre.name)}`}
            className="group relative flex aspect-[4/3] items-end overflow-hidden rounded-sm border border-gold-600/30 sm:aspect-square"
          >
            <Image
              src={genre.image}
              alt={genre.name}
            ...
            <span className="relative z-10 w-full p-3 text-center font-display text-sm font-semibold tracking-wide text-parchment-50">
              {genre.name}
            </span>
          </Link>
        ))}
